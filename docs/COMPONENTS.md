...
| `SyncButton.svelte` | `src/lib/components/layout/SyncButton.svelte` | Sync action props | Triggers git push/pull workflow | Toolbar |
...
| `OperationLogPanel.svelte` | `src/lib/components/engagement/OperationLogPanel.svelte` | `workspaceId`, `onClose` | Timeline notes for an engagement | Pentest toolbar |
...
| `DateTimePicker.svelte` | `src/lib/components/ui/DateTimePicker.svelte` | Value and change callback | Shared date/time control wrapping native `datetime-local` behavior | Shared |
...
- `Select.svelte` and `DateTimePicker.svelte` are the shared input controls for engagement/panel forms
