...
export const PATCH: RequestHandler = async ({ params, request }) => {
  ...
  const { workspaceId, caption, linked_note_path, newFilename } = body as Record<string, unknown>;
  ...
  if (newFilename !== undefined) {
    if (typeof newFilename !== 'string') return error(400, 'newFilename must be a string');
    const trimmed = newFilename.trim().replace(/\.[a-z0-9]+$/i, ''); // strip trailing extension defensively
    if (!trimmed || trimmed.length > 100) return error(400, 'newFilename must be 1–100 characters');
    if (!SAFE_NEW_BASENAME.test(trimmed)) return error(400, 'newFilename contains invalid characters');
    ...
    if (wouldBe !== filename) {
      try {
        resultFilename = await renameScreenshotFile(filename, normalized, workspaceId.trim());
      } catch (e) {
  ...
  try {
    ...
    return json({ filename: resultFilename, url: `/api/screenshots/${resultFilename}`, ...patch });
  } catch (err) {
    console.error('Failed to update screenshot metadata:', { filename, workspaceId, error: err });
...
export const DELETE: RequestHandler = async ({ params, url }) => {
  ...
  try {
    ...
    if ((e as NodeJS.ErrnoException).code === 'ENOENT') return error(404, 'Screenshot not found');
