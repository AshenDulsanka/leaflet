...
const tempRoot = join(tmpdir(), `leaflet-screenshot-file-route-test-${randomUUID()}`);
...
describe('screenshots [filename] GET', () => {
  it('serves a screenshot file with the expected mime type', async () => {
    ...
    const response = await GET({ params: { filename } } as Parameters<typeof GET>[0]);
  ...
  it('rejects invalid filenames with 400', async () => {
    await expectHttpStatus(
      () => GET({ params: { filename: '../evil.png' } } as Parameters<typeof GET>[0]),
      400
    );
  });
});
...
describe('screenshots [filename] PATCH', () => {
  ...
  it('rejects linked_note_path traversal attempts', async () => {
    ...
    await expectHttpStatus(
      () =>
        PATCH({
          params: { filename: '1111111111.png' },
          request: jsonPatchRequest({ workspaceId, linked_note_path: '../secrets.md' }),
        } as Parameters<typeof PATCH>[0]),
      400
    );
  });
  ...
  it('updates caption and linked_note_path metadata', async () => {
    ...
    const row = db
      ...
      .get(workspaceId, filename) as { caption: string; linked_note_path: string } | undefined;
...
describe('screenshots [filename] DELETE', () => {
  it('deletes file and metadata row', async () => {
    ...
    const response = await DELETE({ params: { filename } } as Parameters<typeof DELETE>[0]);
