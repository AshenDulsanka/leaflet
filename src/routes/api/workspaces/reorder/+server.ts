...
export const PATCH: RequestHandler = async ({ request, locals }) => {
  ...
  if (!Array.isArray(body.order)) {
    return json(
      { error: "order must be an array of workspace IDs" },
      { status: 400 },
    );
  }
  ...
  if (body.order.length > MAX_WORKSPACE_COUNT) {
    return json(
      { error: `order exceeds maximum of ${MAX_WORKSPACE_COUNT} items` },
      { status: 400 },
    );
  }
  ...
  for (const item of order) {
    if (typeof item !== "string" || !item.trim()) {
      return json(
        { error: "Each item in order must be a non-empty string workspace ID" },
        { status: 400 },
      );
    }
  }
  ...
  for (const id of ids) {
    ...
    if (!row) {
      return json(
        { error: "One or more workspace IDs not found" },
        { status: 404 },
      );
    }
  }
