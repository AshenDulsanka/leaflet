...
export const PATCH: RequestHandler = async ({ params, request, locals }) => {
  ...
  const allowed = ['value', 'host_id', 'flag_type', 'capture_method', 'screenshot_path', 'submitted', 'notes'];
...
export const DELETE: RequestHandler = async ({ params, locals }) => {
  const { db } = locals;
  const result = db.prepare('DELETE FROM flags WHERE id = ? AND workspace_id = ?').run(params.flagId, params.id);
