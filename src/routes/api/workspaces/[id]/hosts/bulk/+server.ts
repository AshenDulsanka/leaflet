...
export const POST: RequestHandler = async ({ params, request, locals }) => {
  ...
  if (
    ...
    return json({ error: '"raw" field is required and must be a non-empty string' }, { status: 400 });
  }
  ...
  const insertHost = db.prepare<[string, string, string, string, string, string, string]>(`
  ...
  const insertPort = db.prepare<[string, string, number, string, string, string, string]>(`
  ...
  db.transaction(() => {
    ...
    for (const nmapHost of parseResult.hosts) {
      const existingHost = selectHost.get(nmapHost.ip, params.id!) as { id: string } | undefined;
      ...
      if (existingHost) {
        ...
        hostId = randomUUID();
        insertHost.run(hostId, params.id!, nmapHost.ip, nmapHost.hostname, nmapHost.os, now, now);
        imported++;
      }

      for (const port of nmapHost.ports) {
        const existingPort = selectPort.get(hostId, port.number, port.protocol) as
          | { id: string }
          | undefined;

        if (existingPort) {
          updatePort.run(port.service, port.version, port.state, existingPort.id);
        } else {
          const portId = randomUUID();
          insertPort.run(portId, hostId, port.number, port.protocol, port.service, port.version, port.state);
        }
        portCount++;
      }
    }
