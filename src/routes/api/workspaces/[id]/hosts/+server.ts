...
export const GET: RequestHandler = async ({ params, locals }) => {
  ...
  const result = hosts.map((h) => {
    const raw = JSON.parse(h.ports_json as string) as (Record<string, unknown> | null)[];
...
export const POST: RequestHandler = async ({ params, request, locals }) => {
  ...
  const host = db.prepare('SELECT * FROM hosts WHERE id = ?').get(id) as Record<string, unknown>;
