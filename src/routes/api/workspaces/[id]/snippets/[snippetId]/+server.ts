...
export const DELETE: RequestHandler = async ({ params, locals }) => {
  ...
  const result = db
    .prepare('DELETE FROM command_snippets WHERE id = ? AND (workspace_id = ? OR workspace_id IS NULL)')
...
export const PATCH: RequestHandler = async ({ params, request, locals }) => {
  ...
  const { title, command, category, description } = body as Record<string, unknown>;

  if (title === undefined && command === undefined && category === undefined && description === undefined) {
  ...
  if (title !== undefined && (title as string).trim().length > MAX_TITLE_LEN) {
    return json({ error: `title must not exceed ${MAX_TITLE_LEN} characters` }, { status: 400 });
  }
  if (command !== undefined && (typeof command !== 'string' || !command.trim())) {
    return json({ error: 'command must be a non-empty string' }, { status: 400 });
  }
  if (command !== undefined && (command as string).trim().length > MAX_COMMAND_LEN) {
    return json({ error: `command must not exceed ${MAX_COMMAND_LEN} characters` }, { status: 400 });
  }
  if (category !== undefined && typeof category !== 'string') {
    return json({ error: 'category must be a string' }, { status: 400 });
  }
  if (category !== undefined && (category as string).length > MAX_CATEGORY_LEN) {
    return json({ error: `category must not exceed ${MAX_CATEGORY_LEN} characters` }, { status: 400 });
  }
  ...
  if (description !== undefined && (description as string).length > MAX_DESC_LEN) {
    return json({ error: `description must not exceed ${MAX_DESC_LEN} characters` }, { status: 400 });
  }

  const fields: string[] = [];
  const values: (string | null)[] = [];

  if (title !== undefined) { fields.push('title = ?'); values.push((title as string).trim()); }
  if (command !== undefined) { fields.push('command = ?'); values.push((command as string).trim()); }
  if (category !== undefined) { fields.push('category = ?'); values.push(category as string); }
  if (description !== undefined) { fields.push('description = ?'); values.push(description as string); }
  ...
  const result = db
    .prepare(`UPDATE command_snippets SET ${fields.join(', ')} WHERE id = ? AND workspace_id = ?`)
    .run(...values);
