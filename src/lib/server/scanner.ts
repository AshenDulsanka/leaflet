...
export function detectScannerFormat(
  raw: string,
): "nessus" | "burp" | "unknown" {
...
export function parseNessus(xml: string): ScannerParseResult {
  ...
  for (const hostBlock of hostBlocks) {
    ...
    const hostIp =
      attr(openTag, "name") || tagText(hostBlock, 'tag name="host-ip"');

    const itemBlocks =
      hostBlock.match(/<ReportItem\b[\s\S]*?<\/ReportItem>/g) ?? [];
...
export function parseScanner(xml: string): ScannerParseResult {
  ...
  return {
    ...
    errors: [
      {
        message:
          "Unrecognised format. Upload a .nessus or Burp Suite XML export.",
      },
    ],
  };
}
