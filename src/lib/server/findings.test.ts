...
process.env.NOTES_DATA_DIR = join(tmpdir(), `leaflet-findings-test-${process.pid}`);
...
describe('migration v8 – findings table', () => {
  ...
  it('creates the workspace index', () => {
    const db = getDb();
    const row = db
      .prepare(`SELECT name FROM sqlite_master WHERE type='index' AND name='idx_findings_workspace'`)
      .get() as { name: string } | undefined;
  ...
  it('creates the status index', () => {
    const db = getDb();
    const row = db
      .prepare(`SELECT name FROM sqlite_master WHERE type='index' AND name='idx_findings_status'`)
      .get() as { name: string } | undefined;
  ...
  it('creates the host index', () => {
    const db = getDb();
    const row = db
      .prepare(`SELECT name FROM sqlite_master WHERE type='index' AND name='idx_findings_host'`)
      .get() as { name: string } | undefined;
