...
describe("saveScreenshot", () => {
  it("writes a screenshot file and normalizes the extension", async () => {
    ...
    const contents = await fs.readFile(
      join(getScreenshotsDir(), filename),
      "utf-8",
    );
...
describe("insertScreenshotMetadata", () => {
  it("creates a metadata row for a new screenshot", () => {
    ...
    const row = db
      .prepare(
        `SELECT filename, caption, linked_note_path FROM screenshot_metadata WHERE workspace_id = ?`,
      )
      .get(wsId) as
      | { filename: string; caption: string; linked_note_path: string }
      | undefined;
  ...
  it("is idempotent (INSERT OR IGNORE) for the same workspace+filename", () => {
    ...
    const rows = db
      .prepare(
        `SELECT id FROM screenshot_metadata WHERE workspace_id = ? AND filename = ?`,
      )
...
describe("updateScreenshotMetadata", () => {
  ...
  it("updates caption", () => {
    ...
    const row = db
      .prepare(
        `SELECT caption FROM screenshot_metadata WHERE workspace_id = ? AND filename = ?`,
      )
  ...
  it("updates linked_note_path", () => {
    ...
    updateScreenshotMetadata("link.png", wsId, {
      linked_note_path: "folder/my-note.md",
    });

    const db = getDb();
    const row = db
      .prepare(
        `SELECT linked_note_path FROM screenshot_metadata WHERE workspace_id = ? AND filename = ?`,
      )
