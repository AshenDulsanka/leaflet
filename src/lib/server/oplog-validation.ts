...
function unknownFieldError(body: JsonObject, allowedKeys: readonly string[]): ValidationFailure | null {
...
function parseCategory(value: unknown): ValidationResult<OpLogCategory> {
  if (typeof value !== 'string' || !VALID_CATEGORIES.has(value as OpLogCategory)) {
    return { ok: false, error: 'category must be a valid operation log category' };
  }
...
export function parseRouteId(value: string | undefined, label: string): ValidationResult<string> {
  if (typeof value !== 'string') {
...
export function parseOpLogCreateBody(value: unknown): ValidationResult<OpLogCreateBody> {
  if (!isJsonObject(value)) {
    return { ok: false, error: 'Body must be a JSON object' };
  }

  const unknownField = unknownFieldError(value, ['category', 'description', 'host_id', 'timestamp']);
  if (unknownField) {
    return unknownField;
  }

  const category = value.category === undefined ? success<OpLogCategory>('other') : parseCategory(value.category);
  if (!category.ok) {
    return category;
  }
  ...
  const hostId = value.host_id === undefined ? success<string | null>(null) : parseHostId(value.host_id);
  if (!hostId.ok) {
    return hostId;
  }
...
export function parseOpLogPatchBody(value: unknown): ValidationResult<OpLogPatchBody> {
  if (!isJsonObject(value)) {
    return { ok: false, error: 'Body must be a JSON object' };
  }
