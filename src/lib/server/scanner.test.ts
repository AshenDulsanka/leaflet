...
process.env.NOTES_DATA_DIR = join(tmpdir(), `leaflet-scanner-test-${process.pid}`);
...
describe('detectScannerFormat', () => {
  ...
  it('returns nessus for NessusClientData (no _v2)', () => {
    expect(detectScannerFormat('<NessusClientData><Report></Report></NessusClientData>')).toBe('nessus');
  });
  ...
  it('returns unknown for <nmaprun> XML', () => {
    expect(detectScannerFormat('<?xml version="1.0"?><nmaprun>')).toBe('unknown');
  });
...
describe('parseNessus — severity mapping', () => {
  ...
  it('severity=4 maps to critical', () => {
    expect(parseNessus(makeSeverityXml('4')).findings[0].severity).toBe('critical');
  });

  it('severity=2 maps to medium', () => {
    expect(parseNessus(makeSeverityXml('2')).findings[0].severity).toBe('medium');
  });
...
describe('parseBurp — multi', () => {
  it('parses two findings from BURP_MULTI', () => {
    ...
    expect(result.findings[1].title).toBe('Password field with autocomplete enabled');
  });
});
