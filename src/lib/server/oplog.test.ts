...
process.env.NOTES_DATA_DIR = join(
  tmpdir(),
  `leaflet-oplog-test-${process.pid}`,
);
...
describe("migration v7 – operation_log table", () => {
  ...
  it("creates the workspace index", () => {
    ...
    const row = db
      .prepare(
        `SELECT name FROM sqlite_master WHERE type='index' AND name='idx_oplog_workspace'`,
      )
      .get() as { name: string } | undefined;
  ...
  it("creates the timestamp index", () => {
    ...
    const row = db
      .prepare(
        `SELECT name FROM sqlite_master WHERE type='index' AND name='idx_oplog_timestamp'`,
      )
      .get() as { name: string } | undefined;
  ...
  it("creates the host index", () => {
    ...
    const row = db
      .prepare(
        `SELECT name FROM sqlite_master WHERE type='index' AND name='idx_oplog_host'`,
      )
      .get() as { name: string } | undefined;
