...
import { detectFormat, parseNmap, parseNmapGrepable, parseNmapXml } from '$lib/server/nmap.js';
...
describe('detectFormat', () => {
  it('detects grepable format from # Nmap header', () => {
    expect(detectFormat('# Nmap 7.94 scan initiated\nHost: 10.0.0.1 ()')).toBe('grepable');
  });

  it('detects grepable format from ^Host: line', () => {
    expect(detectFormat('Host: 192.168.1.1 ()\nPorts: 22/open/tcp//ssh//')).toBe('grepable');
  });
...
describe('parseNmapGrepable – single host', () => {
  ...
  it('parses open ports correctly', () => {
    ...
    expect(ports[0]).toMatchObject({ number: 22, protocol: 'tcp', service: 'ssh', state: 'open' });
    expect(ports[1]).toMatchObject({ number: 80, protocol: 'tcp', service: 'http', state: 'open' });
  });

  it('normalises open|filtered state to filtered', () => {
    const input = 'Host: 10.0.0.2 ()\nPorts: 8080/open|filtered/tcp//http-proxy///\n';
