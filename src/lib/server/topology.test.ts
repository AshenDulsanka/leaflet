...
process.env.NOTES_DATA_DIR = join(tmpdir(), `leaflet-topology-test-${process.pid}`);
...
describe('migration v10 – topology schema', () => {
  it('adds topo_x column to hosts', () => {
    const db = getDb();
    const cols = db.prepare(`PRAGMA table_info(hosts)`).all() as { name: string }[];
  ...
  it('creates the topology_edges table', () => {
    const db = getDb();
    const row = db
      .prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='topology_edges'`)
...
describe('topology edges CRUD', () => {
  ...
  it('inserts and retrieves a topology edge', () => {
    ...
    const row = db
      ...
      .get(edgeId) as { source_host_id: string; target_host_id: string; label: string } | undefined;
  ...
  it('updates host canvas position (topo_x, topo_y)', () => {
    db.prepare(
      `UPDATE hosts SET topo_x = ?, topo_y = ? WHERE id = ?`
    ).run(123.5, 456.0, hostAId);
