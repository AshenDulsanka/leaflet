...
describe("NetworkTopologyPanel", () => {
  ...
  beforeEach(() => {
    const fetchMock = vi.fn(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (
          url === `/api/workspaces/${workspaceId}/topology` &&
          method === "GET"
        ) {
          return jsonResponse({
            hosts: [
        ...
        if (
          url === `/api/workspaces/${workspaceId}/topology/edges/${edgeId}` &&
          method === "DELETE"
        ) {
        ...
        throw new Error(
          `Unexpected fetch call in NetworkTopologyPanel.test.ts: ${method} ${url}`,
        );
      },
    );
  ...
  it("applies Escape precedence confirm -> selected edge -> close panel", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByRole("dialog", { name: "Delete Connection" }),
      ).toBeNull();
    });
