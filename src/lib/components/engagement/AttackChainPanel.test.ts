...
describe('AttackChainPanel', () => {
  ...
  beforeEach(() => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url === `/api/workspaces/${workspaceId}/nodes` && method === 'GET') {
        return jsonResponse({
          nodes: [
            {
              id: nodeId,
      ...
      if (url === `/api/workspaces/${workspaceId}/nodes/${nodeId}` && method === 'DELETE') {
        return jsonResponse({ ok: true });
      }

      throw new Error(`Unexpected fetch call in AttackChainPanel.test.ts: ${method} ${url}`);
    });
  ...
  it('closes add-step modal on Escape before panel close', async () => {
    ...
    expect(screen.getByRole('dialog', { name: 'Add Attack Step' })).toBeTruthy();

    await fireEvent.keyDown(screen.getByRole('dialog', { name: 'Add Attack Step' }), { key: 'Escape' });
  ...
  it('closes delete confirmation on Escape before panel close', async () => {
    ...
    expect(screen.getByRole('dialog', { name: 'Delete Attack Step' })).toBeTruthy();

    await fireEvent.keyDown(screen.getByRole('dialog', { name: 'Delete Attack Step' }), { key: 'Escape' });
  ...
  it('opens delete confirm, cancel keeps node, confirm deletes node', async () => {
    ...
    const firstConfirmDialog = screen.getByRole('dialog', { name: 'Delete Attack Step' });
    await fireEvent.click(within(firstConfirmDialog).getByRole('button', { name: 'Cancel' }));

    expect(hasDeleteCall(fetchMock, `/api/workspaces/${workspaceId}/nodes/${nodeId}`)).toBe(false);
    expect(screen.getByText('Initial foothold')).toBeTruthy();

    await fireEvent.click(screen.getByRole('button', { name: 'Delete node' }));

    const secondConfirmDialog = screen.getByRole('dialog', { name: 'Delete Attack Step' });
    await fireEvent.click(within(secondConfirmDialog).getByRole('button', { name: 'Delete' }));

    await waitFor(() => {
      expect(hasDeleteCall(fetchMock, `/api/workspaces/${workspaceId}/nodes/${nodeId}`)).toBe(true);
    });
    expect(methodAndUrlCallCount(fetchMock, 'DELETE', `/api/workspaces/${workspaceId}/nodes/${nodeId}`)).toBe(1);
  ...
  it.each([
    ...
    async ({ nodeType, nodeLabel }) => {
      const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        throw new Error(`Unexpected fetch call in AttackChainPanel.test.ts table case: ${method} ${url}`);
      });
      ...
      expect(methodAndUrlCallCount(fetchMock, 'GET', `/api/workspaces/${workspaceId}/nodes`)).toBe(1);
    },
  );
});