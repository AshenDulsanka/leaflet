...
function mockRect(element: Element, top: number, height = 100): void {
  vi.spyOn(element, "getBoundingClientRect").mockReturnValue(
    makeRect(top, height) as DOMRect,
  );
}
...
describe("FileTree drag and drop", () => {
  it("reorders file siblings on center drop deterministically", async () => {
    ...
    const source = document.querySelector(
      '[data-filetree-row-path="alpha.md"]',
    ) as HTMLElement;
    const target = document.querySelector(
      '[data-filetree-row-path="beta.md"]',
    ) as HTMLElement;
  ...
  it("moves nested item to root when dropped at top or bottom edge of list", async () => {
    ...
    const source = document.querySelector(
      '[data-filetree-row-path="folder/note.md"]',
    ) as HTMLElement;
  ...
  it("renders nested children with correct visual indentation", async () => {
    ...
    const parentRow = document.querySelector(
      '[data-filetree-row-path="parent"]',
    ) as HTMLElement;
    const childRow = document.querySelector(
      '[data-filetree-row-path="parent/child.md"]',
    ) as HTMLElement;
